...
function drawWatermark(ctx: CanvasRenderingContext2D, canvasWidth: number, canvasHeight: number) {
  ...
  ctx.fillText('오늘의 컬러링', canvasWidth - 10, canvasHeight - 10);
}

// SNS 공유용 이미지 생성 및 공유
export async function shareImage(svg: SVGSVGElement, imageName: string): Promise<'shared' | 'copied'> {
  console.log('[shareImage] 시작', { svg, imageName });

  return new Promise((resolve, reject) => {
    try {
      const svgData = new XMLSerializer().serializeToString(svg);
      console.log('[shareImage] SVG 직렬화 완료, 길이:', svgData.length);
      ...
      const img = new Image();
      img.onload = async () => {
        console.log('[shareImage] 이미지 로드 완료', { width: img.width, height: img.height });
        ...
        if (navigator.share && navigator.canShare?.({ files: [file] })) {
          try {
            ...
            console.log('[shareImage] 공유 완료');
            resolve('shared');
          } catch (shareError: unknown) {
            // 사용자가 공유 시트를 취소한 경우
            if (shareError instanceof Error && shareError.name === 'AbortError') {
              console.log('[shareImage] 사용자가 공유를 취소했습니다.');
              const abortErr = new Error('사용자가 공유를 취소했습니다.');
          ...
          try {
            ...
            console.log('[shareImage] 클립보드 복사 완료');
            resolve('copied');
          } catch (clipboardError) {
            console.error('[shareImage] 클립보드 복사 실패', clipboardError);
            reject(clipboardError);
          }
        }
      };
      img.onerror = (e) => {
        console.error('[shareImage] 이미지 로드 실패', e);
        URL.revokeObjectURL(svgUrl);
        reject(new Error('이미지 로드 실패'));
      };
      img.src = svgUrl;
    } catch (error) {
      console.error('[shareImage] 예외 발생', error);
      reject(error);
    }
  });
}
